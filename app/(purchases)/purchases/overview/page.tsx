...
const PurchaseOverview = () => {
  ...
  // Use overview slice selectors instead of purchase slice
  ...
  // Transform overview API data to match your Order type
  ...
  useEffect(() => {
    ...
    dispatch(fetchOverviewWithCurrentFilters())
  }, [dispatch])
  ...
  const getSupplierName = (supplierId: number) => {
    ...
    // or modify the overview endpoint to include supplier names
  ...
  useEffect(() => {
    ...
    if (purchaseOrders && purchaseOrders.length > 0) {
      const transformedOrders: Order[] = purchaseOrders.map((purchase, index) => ({
        sn: index + 1,
        purchaseId: `#PUR${purchase.purchaseOrderId.toString().padStart(5, "0")}`,
        supplier: getSupplierName(purchase.supplierId),
        itemPurchased: `${calculateTotalItems(purchase.purchaseOrderItems)} items`,
        payment70: formatCurrency(purchase.totalAmount || 0),
        orderStatus: purchase.orderStatus || purchase.status || "CREATED",
                  ...
                  {loading ? (
                    ...
                    >
                      ...
                      <p className="text-base font-bold text-[#202B3C]">
                        {searchText ? "No purchases match your search." : "No purchases found."}
                      </p>
                    </motion.div>
                  ) : (
                    <>
                                    ...
                                    <div className="flex">
                                      ...
                                      >
                                        <span
                                          ...
                                          style={dotStyle(order.orderStatus)}
                                        ></span>
                                        {order.orderStatus}
                                      </motion.div>
                                    </div>
