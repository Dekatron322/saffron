...
const ReorderCreation = () => {
  ...
  const creatingOrder = useAppSelector(selectCreatingOrder)
  ...
  useEffect(() => {
    ...
    // Fallback: ensure low stock items are available if navigating from elsewhere
  ...
  useEffect(() => {
    // Set selected supplier based on URL parameter
  ...
  // Guard to ensure we only initialize purchase orders once
  const hasInitializedOrders = useRef(false)

  useEffect(() => {
    if (hasInitializedOrders.current) return

    // Initialize purchase orders with selected products when data is ready
    if (productIds.length > 0 && selectedSupplier) {
      // Try to find products from the general products slice first
      let selectedProducts: any[] = products.filter(
        (product) => productIds.includes(product.productId) && product.supplierId === selectedSupplier.id
      )

      // Fallback: if not all products are found, try from lowStockItems (source of reorder selection)
      const foundIds = new Set(selectedProducts.map((p) => p.productId))
      const missingIds = productIds.filter((id) => !foundIds.has(id))

      if (missingIds.length > 0 && lowStockItems.length > 0) {
        const supplierGroup = lowStockItems.find((s) => s.supplierId === selectedSupplier.id)
        if (supplierGroup) {
          const fallbackProducts = supplierGroup.products.filter((p) => missingIds.includes(p.productId))
          selectedProducts = [...selectedProducts, ...fallbackProducts]
        }
      }

      if (selectedProducts.length > 0) {
        const initialOrders: PurchaseOrderItem[] = selectedProducts.map((product: any, index: number) => {
          const category = categories.find((cat) => cat.catId === product.category?.catId) || { catId: 0, catName: "" }
          ...
          return {
            productForm: {
              ...
              batchDetails: {
                mrp: product.batchDetailsDtoList?.[0]?.mrp?.toString() || product.mrp?.toString() || "0",
                batchNo: product.batchDetailsDtoList?.[0]?.batchNo || product.batchNo || "",
                mfg: product.batchDetailsDtoList?.[0]?.mfg || product.mfg || "",
                mfgDate: product.batchDetailsDtoList?.[0]?.mfgDate || product.mfgDate || "",
                expDate: product.batchDetailsDtoList?.[0]?.expDate || product.expDate || "",
                packing: product.batchDetailsDtoList?.[0]?.packing || product.packing || "Box",
              },
              ...
              inclusiveOfTax: product.inclusiveOfTax || false,
            ...
            isMinimized: index !== 0, // Only the first product is expanded initially
          }
        })

        setPurchaseOrders(initialOrders)
        setExpandedIndex(0) // Expand the first product
        hasInitializedOrders.current = true
      }
    }
  }, [productIds, products, selectedSupplier, categories, lowStockItems])
  ...
  const handleInputChange = (
    ...
    index: number,
    fieldType?: "productForm" | "quantity" | "batchDetails"
  ) => {
    ...
    if (fieldType === "quantity") {
      // Handle quantity field directly on the order
      updatedOrders[index] = {
        ...currentOrder,
        quantity: parseInt(value) || 1,
      }
    } else if (fieldType === "batchDetails" && name) {
      // Handle batch details fields
      updatedOrders[index] = {
        ...currentOrder,
        productForm: {
          ...currentOrder.productForm,
          batchDetails: {
            ...currentOrder.productForm.batchDetails,
            [name]: type === "number" ? parseFloat(value) || 0 : value,
          },
        },
      }
    } else {
      // Handle regular product form fields
      if (type === "checkbox") {
        updatedOrders[index] = {
          ...currentOrder,
          productForm: {
  ...
  // Helper to create a fresh empty order item
  const createEmptyOrder = (): PurchaseOrderItem => ({
    productForm: {
      productName: "",
      description: "",
    ...
    quantity: 1,
    isMinimized: true, // New items are minimized by default
  })
  ...
  const removeOrder = (index: number) => {
    ...
    // If no orders left, insert a fresh empty one to keep form usable
    if (updatedOrders.length === 0) {
      updatedOrders.push(createEmptyOrder())
    }
    setPurchaseOrders(updatedOrders)
    ...
    try {
      ...
      const orderData = {
        ...
        products: purchaseOrders.map((order) => {
          // Omit category to align with PurchaseOrderProduct typing in reorderSuggestionSlice
          const { category: _omitCategory, ...productWithoutCategory } = order.productForm as any
          return {
      ...
      const result = await dispatch(createPurchaseOrder(orderData)).unwrap()

      if (result.success) {
        const successMessage =
          result.message ||
          `Successfully created reorder with ${purchaseOrders.length} product${purchaseOrders.length !== 1 ? "s" : ""}!`

        setSuccess(successMessage)
        notify("success", successMessage)

        // Redirect back to reorder suggestions after a short delay
        setTimeout(() => {
      ...
      const errorMessage = err.message || "Failed to create reorder"
  ...
  if (suppliersLoading || productsLoading || categoriesLoading) {
    return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              ...
                              >
                                ...
                                Product {index + 1}: {order.productForm.productName || "New Product"}
                              </h3>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                          <ButtonModule
                            ...
                            variant="primary"
                            size="md"
                            disabled={loading || !selectedSupplier || creatingOrder}
                          >
                            {creatingOrder ? "Creating..." : `Create Reorder`}
                          </ButtonModule>
                        </motion.div>
                      </div>
                    </motion.div>
