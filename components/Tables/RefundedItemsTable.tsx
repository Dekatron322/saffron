...
const RefundedItemsTable = () => {
  ...
  const handleConfirmDelete = (reason: string) => {
    ...
    setTimeout(() => {
      setOrders((prev) => prev.filter((o) => o.returnId !== selectedOrder?.returnId))
  ...
  return (
    <div className="flex-3 mt-3 flex flex-col rounded-md  bg-white ">
      ...
      <div className="items-center justify-between border-b py-2 md:flex md:py-4">
        <p className="text-lg font-semibold md:text-xl">Recent Activities</p>
      </div>
            ...
            <table className="w-full min-w-[800px] border-separate border-spacing-0 text-left">
              <thead>
                <tr className="bg-[#F4F9F8]">
                  ...
                  <th className="cursor-pointer border-b p-4 text-sm" onClick={() => toggleSort("returnId")}>
                  ...
                  <th className="cursor-pointer border-b p-4 text-sm" onClick={() => toggleSort("status")}>
                    <div className="flex items-center gap-2">
                      Status <RxCaretSort />
                    </div>
                  </th>
                  <th className="cursor-pointer border-b p-4 text-sm" onClick={() => toggleSort("reason")}>
                    <div className="flex items-center gap-2">
                      reason <RxCaretSort />
                    </div>
                  </th>
                ...
                {currentOrders.map((order, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="whitespace-nowrap border-b px-4 py-3 text-sm">
                      <div className="flex">
                        <div
                          style={getPaymentStyle(order.quantity)}
                    ...
                    <td className="whitespace-nowrap border-b px-4 py-3 text-sm">
                      <div className="flex">
                        <div
                          ...
                          className="flex items-center justify-center gap-1 rounded-full px-2 py-1"
                        >
                          <span className="size-2 rounded-full" style={dotStyle(order.status)} />
                          {order.status}
                        </div>
                      </div>
                    </td>
                    <td className="border-b px-4 py-3 text-sm">
                      <div className="flex items-center gap-2">{order.reason}</div>
                    </td>
      ...
      <DeleteModal
        ...
        businessName={selectedOrder?.returnId || ""}
      />
    </div>
  )
}

export default RefundedItemsTable
