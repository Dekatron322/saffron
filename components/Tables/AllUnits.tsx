...
const AllUnits = () => {
  ...
  const exportToCSV = () => {
    ...
    const data = filteredUnits.map((unit) => [unit.unitId, unit.baseUnit, unit.secondaryUnit, unit.shortName])
      ...
      <div className="mb-5 flex w-full justify-between">
        ...
        <div className="flex gap-4">
          <ButtonModule variant="black" size="md" icon={<ExportIcon />} iconPosition="end" onClick={exportToCSV}>
            <p className="max-sm:hidden">Exports</p>
          </ButtonModule>

          <ButtonModule variant="ghost" size="md" icon={<AddBusiness />} iconPosition="end" onClick={toggleAddUnit}>
            <p className="max-sm:hidden"> Add New Unit</p>
          </ButtonModule>
        </div>
      </div>
